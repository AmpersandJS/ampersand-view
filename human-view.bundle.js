...
function StateyBase(attrs, options) {
  attrs || (attrs = {});
  options || (options = {});
...
var extend = function (spec) {
  var parent = this;
  var BaseClass = this._super || StateyBase;
  ...
  Object.defineProperties(Statey.prototype, accessors);

  // Mix in our methods
  _.extend(Statey.prototype, prototypeMixins, {
  ...
  _.extend(Statey._spec, spec);

  _.each(Statey._spec, function (value, key) {
    if (key === 'props' || key === 'session') {
      _.each(value, function (def, name) {
        Statey.prototype._createPropertyDefinition.call(Statey.prototype, name, def, key === 'session');
      });
    } else if (key === 'derived') {
      _.each(value, function (def, name) {
        createDerivedProperty(Statey.prototype, name, def);
      });
    } else if (key === 'collections') {
      Statey.prototype._collections = value;
    } else {
      Statey.prototype[key] = value;
    }
  });
  ...
  Statey._super = Statey;

  // Maintain ability to further extend
  Statey.extend = extend;

  return Statey;
};

// also expose data types in our export
StateyBase.dataTypes = dataTypes;
StateyBase.extend = extend;
